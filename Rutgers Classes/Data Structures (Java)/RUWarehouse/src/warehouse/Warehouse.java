...
public class Warehouse {
    ...
    public void betterAddProduct(int id, String name, int stock, int day, int demand) {
        ...
        int currentSector = id % 10 + 1;

        if (currentSector == 10)
        currentSector = 0; 

        while (currentSector != id % 10) {
            if (sectors[currentSector].getSize() < 5){
                sectors[currentSector].add(new Product(id, name, stock, day, demand));
                fixHeap(currentSector);
                return; 
            }
